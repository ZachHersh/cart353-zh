...
float boxSize; // length of our box (a cube)
...
void draw() {
  ...
  pushMatrix();
  // set our point of origin to the middle of the screen
  ...
  // if thr location of the ball's right is more than the right side of the box
  if ((location.x+sphereR) > width/2 +boxSize/2) {
    // change the irection of horizontal velocity
    velocity.x = velocity.x * -1;
    // set the horizontal location to be the maximum it can be inside the box.
  ...
  // else if the location of the ball's left is less than the left side of the box
    ...
    // change the direction of the horiz velocity
    velocity.x = velocity.x*-1;
    // set the horiz location to be the minimum it can be inside the box
    location.x = width/2-boxSize/2+sphereR;
  }
  
  // if the location of the ball's top is higher than the top of the box
  ...
  //else if the location of the ball's base is lower than the bottom of the box
  ...
  // else if the location of the ball's back sticks out beyond the bac of the box
  ...
  // draw our ball there: a sphereof radius sphereR
  sphere(sphereR);
  popMatrix();
